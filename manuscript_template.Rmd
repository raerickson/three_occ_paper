...
Using $\mu_{\psi} \sim X \beta$ as an example, imagine that we fit an intercept for each specie's detection probability at a site.
For example, the vector $\beta$ looks like: $\beta_\textrm{spp 1}, \beta_\textrm{spp 2}, \ldots \beta_\textrm{spp N}$.
We can then fit a regression to each site $i$, for example, let's say we have $N$ sites:
...
- Because each term coefficient is specices intercept, the correlation between these is the species' correlation across sites.
...
```{r, fig.ncol = 2, out.width="50%", fig.align = "center", fig.cap="Species-level occpuancy correlations .", fig.subcap=c("Simulated (known) correlations", "Estimated correlations")}
knitr::include_graphics("sim_example_figures/known_spp_matrix.jpg")
knitr::include_graphics("sim_example_figures/omega_psi_plot.jpg")
```



```{r, fig.ncol = 2, out.width="50%", fig.align = "center", fig.cap="Detection probability correlations .", fig.subcap=c("Simulated (known) detection correlations", "Estimated correlations")}
knitr::include_graphics("sim_example_figures/known_detection_matrix.jpg")
knitr::include_graphics("sim_example_figures/omega_p_plot.jpg")
```
